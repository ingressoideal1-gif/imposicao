...
DECLARE
    ...
    expected_expression text := '(((''https://ideal-imposition.vercel.app/cliente/''::text || numero_pedido) || ''-''::text) || (token)::text)';
...
BEGIN
    ...
    ALTER TABLE public.pedidos_links_cliente ALTER COLUMN link SET EXPRESSION AS (
        'https://imposition.ai-ideal.com.br/cliente/' || numero_pedido || '-' || token
    );
    SELECT md5(coalesce(string_agg((to_jsonb(t)-'link')::text, '' ORDER BY id), ''))
    ...
    IF EXISTS (SELECT 1 FROM public.pedidos_links_cliente
        WHERE link IS DISTINCT FROM ('https://imposition.ai-ideal.com.br/cliente/' || numero_pedido || '-' || token)) THEN
...
SELECT count(*) AS total,
       count(*) FILTER (WHERE link LIKE 'https://imposition.ai-ideal.com.br/cliente/%') AS dominio_destino
