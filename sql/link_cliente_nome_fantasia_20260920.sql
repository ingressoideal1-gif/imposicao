-- e-deal / vwbtitjlpelrcnsytzqw: somente public.link_cliente_pedido(text,text).
-- Zero linhas comerciais alteradas. Mantem assinatura, privilegios e regras fiscais.
-- Origem: definicao instalada conferida em 20/09/2026; aborta se houver alteracao concorrente.
-- Recuperacao: rpc-antes.json/sql no backup sincronizacao-sessoes-20260920-111043.
BEGIN;
DO $guard$ BEGIN
 IF md5(pg_get_functiondef('public.link_cliente_pedido(text,text)'::regprocedure)) <> '56f59e5ede2e5627687386d86089b606' THEN
  RAISE EXCEPTION 'A definicao do portal mudou; revisar antes de reaplicar';
 END IF;
END $guard$;
CREATE OR REPLACE FUNCTION public.link_cliente_pedido(p_numero text, p_token text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
    ...
    -- Nome comercial exibido, mantendo o cadastro fiscal separado.
    IF v_prop.id_cliente IS NOT NULL THEN
        SELECT c.* INTO v_cli_comercial FROM public.clientes c
         WHERE c.id_cliente = v_prop.id_cliente LIMIT 1;
    END IF;
    ...
    IF v_end.id IS NULL AND v_prop.id_cliente IS NOT NULL THEN
        ...
        IF v_end.id IS NULL THEN
            SELECT e.* INTO v_end
             ...
             WHERE e.id_cliente = v_prop.id_cliente
           AND coalesce(e.obs, '') NOT LIKE 'portal-entrega-pedido:%'
               AND (SELECT count(*) FROM enderecos x WHERE x.id_cliente = v_prop.id_cliente
                     AND coalesce(x.obs, '') NOT LIKE 'portal-entrega-pedido:%') = 1
             LIMIT 1;
        END IF;
        ...
        IF v_end_fat.id IS NULL THEN
           ...
           AND coalesce(e.obs, '') NOT LIKE 'portal-entrega-pedido:%'
               AND (SELECT count(*) FROM enderecos x
                     WHERE x.id_cliente = COALESCE(v_prop.id_faturado, v_prop.id_cliente)
                       AND coalesce(x.obs, '') NOT LIKE 'portal-entrega-pedido:%') = 1
             LIMIT 1;
        END IF;
    END IF;
    ...
    RETURN jsonb_build_object(
        'pedido', jsonb_build_object(
            ...
            'cliente',          COALESCE(NULLIF(btrim(v_cli_comercial.fantasia), ''),
                                         NULLIF(btrim(v_cli_comercial.nome), ''), v_prop.cliente),
