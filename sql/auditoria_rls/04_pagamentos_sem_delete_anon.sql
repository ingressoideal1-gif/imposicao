-- REVISAO 04: preserva grants de coluna observados em producao. Aplicacao separada do coletor.
-- Alvo: e-deal / producao / vwbtitjlpelrcnsytzqw / public.pagamentos_v2.
-- Baseline: coleta 20260916T110256417Z-e537c87b, PostgreSQL 17.4; ERP exige login (usuario confirmou).
...
BEGIN
    ...
    IF NOT pg_catalog.has_table_privilege('anon', alvo, 'SELECT')
       ...
       OR pg_catalog.has_table_privilege('anon', alvo, 'INSERT')
       OR pg_catalog.has_table_privilege('anon', alvo, 'UPDATE') THEN
    ...
    -- ACLs de coluna sao independentes dos grants de tabela e devem permanecer identicos.
    SELECT jsonb_agg(jsonb_build_object('coluna',a.attname,'acl',a.attacl::text) ORDER BY a.attnum)
    INTO colunas_antes FROM pg_catalog.pg_attribute a
    WHERE a.attrelid=alvo AND a.attnum>0 AND NOT a.attisdropped;

    REVOKE DELETE ON TABLE public.pagamentos_v2 FROM anon;

    SELECT jsonb_agg(jsonb_build_object('coluna',a.attname,'acl',a.attacl::text) ORDER BY a.attnum)
    INTO colunas_depois FROM pg_catalog.pg_attribute a
    WHERE a.attrelid=alvo AND a.attnum>0 AND NOT a.attisdropped;
    IF colunas_antes IS DISTINCT FROM colunas_depois THEN
        RAISE EXCEPTION 'ACLs de coluna alterados indevidamente';
    END IF;
