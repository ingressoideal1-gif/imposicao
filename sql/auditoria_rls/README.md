...
`06_bancos_conter_escrita_anon.proposta.sql` é a contenção intermediária que mantém leitura anônima. `07_bancos_fechar_anon_e_habilitar_rls.proposta.sql` é a proposta de fechamento completo depois da migração dos consumidores: remove todos os privilégios de `anon`, habilita RLS sem policy direta e preserva o portal pela RPC `link_cliente_bancos_modelos(text,text)` SECURITY DEFINER. Nenhuma das duas foi aplicada. A revisão 07 exige estações aplicáveis em NewProd 1.2.335, baseline de tráfego comparado, snapshot novo, aviso de alvo/efeito/janela e aceite expresso do mantenedor.
