...
BEGIN
 ...
 IF EXISTS (SELECT 1 FROM validacao_ic.producao_acesso_credenciais) THEN RAISE EXCEPTION 'Teste deveria ativar sem ingressos'; END IF;
 IF has_function_privilege('anon','validacao_ic.producao_acesso_ativar_qr_evento(text,text,text,text,uuid)','EXECUTE')
