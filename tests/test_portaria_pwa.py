...
# O registro do service worker saiu de dentro do `portaria.html` em 16/08/2026 e
# virou `sw-registro.js`, comum as tres telas -- a casa precisava dele para o
# navegador oferecer instalar. A faixa continua sendo da portaria: e la que ela
# e declarada, e o modulo so a usa se existir.
...
def test_a_tela_avisa_quando_ha_versao_nova():
    ...
    assert 'id="faixa-atualizacao"' in _ler("frontend/portaria.html")
    assert "updatefound" in _ler("frontend/sw-registro.js")
...
def test_a_atualizacao_nunca_recarrega_sozinha():
    ...
    js = _ler("frontend/sw-registro.js")
    assert "location.reload" in js
    trecho = js[js.index("faixa-atualizacao"):]
...
def test_o_aviso_nao_aparece_na_primeira_instalacao():
    ...
    assert "navigator.serviceWorker.controller" in _ler("frontend/sw-registro.js")
