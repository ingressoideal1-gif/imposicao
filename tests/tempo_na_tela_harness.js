...
const PAGINA = '<!doctype html><meta charset="utf-8">'
    ...
    + 'font-family:system-ui,sans-serif}:root{--text-dim:#94a3b8;--border:#1e293b}'
...
(async () => {
    ...
    const medido = await page.evaluate((codigo, casos) => {
        eval(codigo);
        ...
        corpo.innerHTML = casos.map(c => '<tr>'
            ...
            + '</tr>').join('')
            // A quinta linha e um pedido sem relogio: a tabela ainda nao existe
            // no banco, ou e a primeira vez que ele aparece.
