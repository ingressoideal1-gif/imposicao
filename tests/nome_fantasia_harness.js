...
(async function () {
    ...
    const banco = { from: () => ({ select: () => ({ in: async (_campo, ids) => {
        idsConsultados.push(...ids);
        return { data: [
            { id_cliente: 10, fantasia: 'Fantasia Um', nome: 'RAZAO UM' },
            { id_cliente: 20, fantasia: '', nome: 'RAZAO DOIS' }
        ], error: null };
    } }) }) };
    await funcoes.aplicarNomesPreferenciaisDasPropostas(banco, propostas);
    ok(idsConsultados.includes('10') && !idsConsultados.includes('99'),
        'consulta o cliente comercial, nao quem apenas fatura', idsConsultados);
