...
(async()=>{
 ...
 try {
  ...
  assert.match(await page.$eval('#resposta-titulo',e=>e.textContent),/AGUARDANDO PUBLICAÇÃO/);
  await page.evaluate(()=>document.getElementById('btn-proximo').click());
  ...
  console.log(JSON.stringify({pendingConfigurable:true,emptyAcceptsCodeWithoutCounting:true,retryAfterPartialDownload:true,sameToken:true,readyOffline:true,pages}));
