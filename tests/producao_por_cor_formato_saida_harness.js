...
(async () => {
    ...
    try {
        for (const savedOutput of [null, 's2']) {
            const results = [];
            for (const external of [false, true]) {
                ...
                await tab.evaluate(({ external, savedOutput }) => {
                    window.state = {
                        osItens: { 'vibe_1': [{ id: 11, _vibe_id_produto: 9, id_produto: 9, produto: 'Modelo',
                            status_impressao: 'Aguardando', formato_id: null, saida_id: savedOutput, quantidade: 100,
            ...
            assert.deepEqual(results[0], { formato: 'f1', saida: savedOutput || 's1', modeloFormato: 'f1', modeloSaida: savedOutput || 's1', next: { formato: 'f2', saida: 's2' } });
