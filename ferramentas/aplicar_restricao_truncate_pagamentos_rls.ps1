<# Aplicador exclusivo da restricao TRUNCATE anon em pagamentos. Sem -Executar: previa. #>
[CmdletBinding()]
param(
    [switch]$Executar,
...
$sqlPathRls = Join-Path $raizRls 'sql\auditoria_rls\05_pagamentos_sem_truncate_anon.sql'
$hashRls = 'F1BF0ACF200E054BAAFAE87DDB0BDB15E66011026D7162B7B6A6B703AA366693'
...
Write-Output 'Efeito: revogar somente TRUNCATE direto de anon. Zero linhas comerciais modificadas.'
...
try {
    ...
    Salvar-ReciboRls 'intencao-truncate-pagamentos' @{
        projeto='vwbtitjlpelrcnsytzqw'; ambiente='producao'; hash_sql=$hashRls;
        efeito='REVOKE TRUNCATE ON public.pagamentos_v2 FROM anon'; linhas_alteradas=0;
        revisao_consumidores=$RevisaoConsumidores; estado='preparado; ainda nao comprova execucao'
    }
    $sqlRls = "SET imposition.pagamentos_truncate_anon_revisado = 'sim';`n" + [IO.File]::ReadAllText($sqlPathRls)
    ...
    # Verificacao independente em consulta somente leitura; nunca testar TRUNCATE em dados reais.
    $verificacaoRls = @"
BEGIN READ ONLY;
SELECT has_table_privilege('anon','public.pagamentos_v2','SELECT') AS anon_le,
       has_table_privilege('anon','public.pagamentos_v2','TRUNCATE') AS anon_esvazia,
       has_table_privilege('authenticated','public.pagamentos_v2','TRUNCATE') AS autenticado_esvazia,
       has_table_privilege('service_role','public.pagamentos_v2','TRUNCATE') AS backend_esvazia;
ROLLBACK;
"@
    ...
    if ($resultadoRls.Count -ne 1 -or $resultadoRls[0].anon_le -ne $true -or $resultadoRls[0].anon_esvazia -ne $false -or $resultadoRls[0].autenticado_esvazia -ne $true -or $resultadoRls[0].backend_esvazia -ne $true) { throw 'Pos-condicoes divergentes.' }
    Salvar-ReciboRls 'resultado-truncate-pagamentos' @{projeto='vwbtitjlpelrcnsytzqw';hash_sql=$hashRls;estado='verificado';privilegios=$resultadoRls[0]}
    Write-Output 'Restricao aplicada e verificada: anon sem TRUNCATE; leituras e demais papeis preservados.'
} catch {
    if ($enviadoRls) {
