...
export function layoutEmailArte(texto: string, portal: string, link?: string, numero?: string) {
  ...
  const linhas = texto.split(/\r?\n/).filter(linha => !/^\s*-{3,}\s*$/.test(linha) &&
    !/^\s*LINK DE APROVAÇÃO INTERATIVA:\s*$/i.test(linha) && (!link || linha.trim() !== link));
  const resumo = linhas.findIndex(linha => /^RESUMO DOS MODELOS DO PEDIDO:/i.test(linha.trim()));
  ...
  const intro = render(resumo >= 0 ? linhas.slice(0, resumo) : linhas);
  const detalhes = resumo >= 0 ? render(linhas.slice(resumo)) : "";
