...
def _generate_barcode(data: str, width_mm: float, height_mm: float, color_hex: str = "#000000", barcode_format: str = "code128") -> bytes:
    """Gera o cÃ³digo de barras como imagem PNG em bytes usando coloraÃ§Ã£o nativa."""
    try:
        import barcode
    ...
    # Assegurar que o formato estÃ¡ em minÃºsculas
    fmt = (barcode_format or "code128").lower()

    # PrÃ©-processamento e formataÃ§Ã£o de dados para simbologias numÃ©ricas estritas
    if fmt in ("ean13", "ean8", "upca", "itf"):
        ...
        if fmt == "ean13":
            # EAN-13 precisa de 12 dÃ­gitos (o 13Âº dÃ­gito de verificaÃ§Ã£o Ã© calculado pela biblioteca)
            if len(clean_data) < 12:
                clean_data = clean_data.zfill(12)
            elif len(clean_data) > 13:
                clean_data = clean_data[:12]
        elif fmt == "ean8":
            # EAN-8 precisa de 7 dÃ­gitos (o 8Âº dÃ­gito de verificaÃ§Ã£o Ã© calculado pela biblioteca)
            if len(clean_data) < 7:
                clean_data = clean_data.zfill(7)
            elif len(clean_data) > 8:
                clean_data = clean_data[:7]
        elif fmt == "upca":
            # UPC-A precisa de 11 dÃ­gitos (o 12Âº dÃ­gito de verificaÃ§Ã£o Ã© calculado pela biblioteca)
            if len(clean_data) < 11:
                clean_data = clean_data.zfill(11)
            elif len(clean_data) > 12:
                clean_data = clean_data[:11]
        elif fmt == "itf":
            # ITF (Interleaved 2 of 5) exige comprimento par de dÃ­gitos
...
def _rotate_rect(rect: fitz.Rect, angle: int, page: fitz.Page) -> tuple[fitz.Rect, fitz.Matrix]:
    """Retorna a matriz de transformaÃ§Ã£o para rotaÃ§Ã£o em torno do centro do rect."""
...
class ImpositionConfig:
    def __init__(self,
        ...
        self.gap_h = formato.get("gap_h_mm", 0) * MM2PT   # espaÃ§o horizontal entre cols
        self.gap_v = formato.get("gap_v_mm", 0) * MM2PT   # espaÃ§o vertical entre rows
        # Deslocamentos e rotaÃ§Ãµes
        self.offset_h = formato.get("offset_h_mm", 0) * MM2PT
        self.offset_v = formato.get("offset_v_mm", 0) * MM2PT
        self.rotations = formato.get("rotations", {})  # DicionÃ¡rio de rotaÃ§Ãµes de cÃ©lulas (ex: {"0": 90})
        ...
        if layout_schema == "pdf_multiple":
            # Para Pdf MÃºltiplo, a quantidade total de itens Ã© baseada na quantidade de pÃ¡ginas
            try:
        ...
        # Elementos VDP da numeraÃ§Ã£o
        self.elements = []
        
        # Carregar numeraÃ§Ã£o 1
        if numeracao and "elements" in numeracao:
            for el in numeracao["elements"]:
                e = dict(el)
                # Converter mm â†’ pt para todos os campos de posiÃ§Ã£o/tamanho
...
class ImpositionEngine:
    ...
    def _load_base_as_pdf(self) -> fitz.Document:
        """Abre o arquivo base (PDF, JPG, PNG) como documento fitz com dimensÃµes fÃ­sicas precisas."""
        ...
        if f.endswith(".pdf"):
            ...
            # Imagem â†’ converter para PDF temporÃ¡rio em memÃ³ria ajustando ao tamanho do item
            ...
            # Calcular dimensÃµes para ajustar proporcionalmente e centralizar (equivalente ao frontend)
    ...
    def _render_element(self, page: fitz.Page, el: dict, cell_x0: float, cell_y0: float, val: int, csv_row: dict | None = None):
        """Renderiza um elemento VDP na posiÃ§Ã£o absoluta da cÃ©lula."""
        el_x = cell_x0 + el["_x"]
        el_y = cell_y0 + el["_y"]
        ...
        if t in ("TEXT", "FIXED"):
            ...
            # Ponto de inserÃ§Ã£o Y em PyMuPDF Ã© baseline; ajustamos pela font_size
            if angle != 0:
                # Para rotaÃ§Ã£o, usamos insert_text com morph
            ...
            if pdf_content:
                try:
                    ...
                    if not isinstance(pdf_content, str) or not pdf_content.strip():
                        print(f"[engine] Elemento PDF ignorado â€” pdf_content invÃ¡lido")
                        return
                ...
                print(f"[engine] Elemento PDF sem pdf_content â€” ignorado")
    ...
    def process(self):
        ...
        # Calcular Ã¡rea total usada na folha (itens + gaps)
        ...
        if used_w > cfg.sheet_w or used_h > cfg.sheet_h:
            err = (
                f"O formato de entrada (Matriz {cols}Ã—{rows}) nÃ£o cabe na folha de saÃ­da! "
                f"NecessÃ¡rio: {used_w/MM2PT:.1f}Ã—{used_h/MM2PT:.1f}mm. "
                f"DisponÃ­vel: {cfg.sheet_w/MM2PT:.1f}Ã—{cfg.sheet_h/MM2PT:.1f}mm."
            )
        ...
        if cfg.layout_schema == "multi_artes":
            ...
            for art in sorted_artes:
                ...
                if local_path and os.path.exists(local_path):
                    try:
                        art_doc = fitz.open(local_path)
                    except Exception as e:
                        print(f"Erro ao abrir arquivo local do PDF da arte: {e}")
                elif pdf_url:
                    art_doc = load_pdf_from_url(pdf_url)
        ...
        for S in range(total_sheets):
            ...
            for row in range(rows):
                for col in range(cols):
                    ...
                    # Determinar a pÃ¡gina do PDF base e elementos para a Frente
                    ...
                    # 1. Criar PDF temporÃ¡rio para renderizar o item + elementos VDP (sem rotaÃ§Ã£o da cÃ©lula inicialmente)
                    temp_doc = fitz.open()
                    temp_page = temp_doc.new_page(width=cfg.item_w, height=cfg.item_h)

                    # Centralizar e aplicar offset no plano da cÃ©lula temporÃ¡ria
                    ...
                    # Inserir arte na pÃ¡gina temporÃ¡ria
                    if current_doc_base:
                    ...
                    for el in current_elements:
                        # Filtrar elementos que sÃ£o apenas para verso
                        if el.get("face", "both") == "back":
                            continue

                        # Mantemos a rotaÃ§Ã£o configurada original do elemento, mas nÃ£o a rotaÃ§Ã£o da cÃ©lula (que serÃ¡ aplicada na folha inteira)
                        rotated_el = dict(el)
                        ...
                        # Renderiza na pÃ¡gina temporÃ¡ria usando coordenadas relativas diretas
                        self._render_element(temp_page, rotated_el, 0, 0, current_val, csv_row)

                    # 2. Impor a pÃ¡gina temporÃ¡ria completa (arte + VDP) na folha final, aplicando a rotaÃ§Ã£o da cÃ©lula
            ...
            if is_duplex:
                ...
                for row in range(rows):
                    for col in range(cols):
                        ...
                        # Para o verso, a coluna fÃ­sica Ã© espelhada horizontalmente
                        ...
                        # Determinar a pÃ¡gina base de verso no PDF de entrada
                        if cfg.layout_schema == "pdf_multiple":
                        ...
                        # PosiÃ§Ã£o fÃ­sica da cÃ©lula de verso na folha final
                        ...
                        # 1. Criar PDF temporÃ¡rio para renderizar o verso do item + elementos VDP
                        temp_doc = fitz.open()
                        temp_page = temp_doc.new_page(width=cfg.item_w, height=cfg.item_h)
                        ...
                        # Desenhar nome da arte no topo da cÃ©lula, se houver
                        if arte_nome:
                        ...
                        for el in current_elements:
                            # Filtrar elementos que sÃ£o apenas para frente
                        ...
                        # 2. Impor a pÃ¡gina temporÃ¡ria de verso na folha final, aplicando a rotaÃ§Ã£o correspondente
