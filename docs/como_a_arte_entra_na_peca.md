...
existe porque a mesma arte é desenhada por quatro programas diferentes, e durante muito tempo
eles não concordavam. O sintoma que isso produz é sempre o mesmo, e é o pior possível numa
gráfica: **a tela mostra uma coisa e o papel sai outra.**
...
## Ao mexer em qualquer uma das quatro janelas
...
4. Não desenhe moldura, contorno ou marca de tela dentro do bitmap.
5. Rode `tests/test_arte_da_amostra_no_tamanho_real.py` e `tests/test_amostra_sem_moldura.py`.
   O primeiro vigia também o `engine.py`: se o motor mudar de regra, ele avisa que as telas
   precisam mudar junto.
6. Publique o agente na mesma leva (`GUIA_AGENTE.md`) — o executável embute uma cópia do
   frontend.
