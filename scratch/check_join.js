...
async function main() {
    // try to fetch a proposta and its pedidos_artes
    const res = await fetch(`${url}/rest/v1/propostas?select=id_int,created_at,pedidos_artes(nome_evento)&limit=1`, {
        headers: { 'apikey': key, 'Authorization': `Bearer ${key}` }
    });
    if (!res.ok) {
        console.log("Join failed", await res.text());
        return;
    }
