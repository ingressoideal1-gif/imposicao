...
for filename in ["frontend/producao.html", "frontend/script.js", "frontend/cliente.js"]:
    p = os.path.join(dir_path, filename)
    if os.path.exists(p):
        with open(p, "r", encoding="utf-8", errors="ignore") as file:
            for idx, line in enumerate(file, 1):
                if any(kw in line.lower() for kw in ["imprimir", "pedgerarpdf", "cancelar", "abortcontroller", "cancel", "gerando"]):
