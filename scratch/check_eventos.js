...
async function main() {
    const res = await fetch(`${url}/rest/v1/eventos?limit=1`, {
        headers: { 'apikey': key, 'Authorization': `Bearer ${key}` }
    });
    if (res.ok) console.log("eventos:", await res.json());

    const res2 = await fetch(`${url}/rest/v1/briefings?limit=1`, {
        headers: { 'apikey': key, 'Authorization': `Bearer ${key}` }
    });
    if (res2.ok) console.log("briefings:", await res2.json());
}
main();
